...
#   general — desktop/gaming, HZ=250, PREEMPT=y, schedutil/reflex (DEFAULT)
#   LTO: none (default), ThinLTO, FullLTO  selectable via lto=None/ThinLTO/FullLTO
#
# Usage:
#   ./build.sh
#   ./build.sh --option N
    ...
    case "$arg" in
        ...
        lto=*)
            ...
            case "${LTO_ARG,,}" in
                ...
                none|no)      LTO_FLAVOR="none" ;;
                *)
                    echo "Unknown LTO flavor: ${LTO_ARG}. Valid: None, ThinLTO, FullLTO"
                    exit 1
                    ;;
            esac
            ;;
...
if [[ -f .config ]]; then
    echo -e "\e[1;34m[*]\e[0m Using existing .config"
elif [[ -f config ]]; then
...
if [[ "$DO_BUILD" == "1" ]]; then
    ...
    LOCALVERSION_SUFFIX="-Strawberry-"
    if [[ "$LTO_FLAVOR" != "none" ]]; then
        LOCALVERSION_SUFFIX="${LOCALVERSION_SUFFIX}$(lto_label)-"
    fi
    ...
    case "$LTO_FLAVOR" in
        ...
        thin)
            ...
            ;;
        none)
            echo -e "\e[1;34m[*]\e[0m LTO disabled (none)..."
            scripts/config --disable CONFIG_LTO_CLANG_THIN
            scripts/config --disable CONFIG_LTO_CLANG_FULL
            ;;
    esac
